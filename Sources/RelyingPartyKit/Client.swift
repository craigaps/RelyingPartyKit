...
/// RelyingPartyClient gives users the ability to authenticate and sign-up to an account.  In additional, users can request a Webauthn challenge to register and verify a fast identity on-line (FIDO) authenticator.
///

/// An RelyingPartyClient object provides a lightweight way to orchestrate user authentication and device registration.
public struct RelyingPartyClient {
    ...
    func register(nickname: String, clientDataJSON: Data, attestationObject: Data, credentialId: Data, token: Token) async throws {
        ...
        let url = baseURL.appendingPathComponent("/v1/registration")
